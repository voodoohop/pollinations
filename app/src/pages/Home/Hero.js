...
function GenerativeImageFeed() {
  ...
  const [showAdvisory, setShowAdvisory] = useState(true); // Added state for showing the advisory
  ...
  return (
    <div style={{ wordBreak: "break-all" }}>
      ...
      <GenerativeImageURLContainer>
        ...
        {/* Show the advisory */}
        {showAdvisory && (
          <AdvisoryText onClick={() => setShowAdvisory(false)}>
            Warning: This feed may contain NSFW and potentially disturbing content. It is unfiltered.
        ...
        <b style={{ whiteSpace: "nowrap" }}>
          <a href={image?.imageURL}>https://image.pollinations.ai/prompt/[prompt]</a>
          <ParamsButton />{" "}
        </b>{" "}
        <br />
        {/* links */}
        Create with ChatGPT:{" "}
        <b>
          <a href="https://chat.openai.com/share/d24ce24f-283a-4f76-bacb-6e0740c234a1">
            ChatGPT
          </a>
          ,{" "}
          <a href="https://www.reddit.com/r/ChatGPT/comments/zktygd/did_you_know_you_can_get_chatgpt_to_generate/">
            Reddit
          </a>
          ,{" "}
          <a href="https://youtu.be/gRP3V2sz-M8?t=55">Youtube</a>
        </b>
        {/* input field */}
        <br />
        <br />
  ...
  return (
    <div>
      <Input
        ...
        style={{ width: "100%" }}
        placeholder="Or type your prompt here (any language)"
      />
      {/* right aligned button */}
      <div style={{ textAlign: "right" }}>
        <Button onClick={() => window.open(`https://image.pollinations.ai/prompt/${prompt}`)}>
          Create
        </Button>
      </div>
    </div>
...
function ParamsButton() {
  ...
  return (
    <Tooltip title="?width=[width]&height=[height]&seed=[seed]">
      <Button
        size="small"
        style={{ minWidth: "16px", display: "inline-block", fontSize: "90%" }}
        onClick={() => setShowParams(!showParams)}
      >
        <span style={{ textTransform: "none" }}>{showParams ? "?width=[width]&height=[height]&seed=[seed]" : "+"}</span>
      </Button>
    </Tooltip>
  );
}
